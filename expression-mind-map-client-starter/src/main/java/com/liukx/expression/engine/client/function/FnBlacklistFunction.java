 ...
 */
@Component
public class FnBlacklistFunction extends AbstractSimpleFunction {
    ...
    public Enum<? extends ExpressFunctionDocumentLoader> documentRegister() {
        return BaseFunctionDescEnum.BLACKLIST;
    }
        ...
        switch (operation) {
            case "add":
                ...
                env.recordTraceDebugContent(getName(), "debug",
                        String.format("黑名单新增: group=%s, key=%s, value=%s, result=%s", group, key, value, result));
                break;
            case "del":
                result = listCheckService.remove(group, key, value);
                env.recordTraceDebugContent(getName(), "debug",
                        String.format("黑名单删除: group=%s, key=%s, value=%s, result=%s", group, key, value, result));
                break;
            default:
                result = listCheckService.contains(group, key, value);
                env.recordTraceDebugContent(getName(), "debug",
                        String.format("黑名单检查: group=%s, key=%s, value=%s, result=%s", group, key, value, result));
                break;
        }
        return result;
    }
